        ...
        visitor: {
            ...
            ImportDefaultSpecifier: function ImportDefaultSpecifier(path, _ref4) {
                ...
                if (matchExtensions.test(value)) {
                    ...
                    var varDeclaration = t.variableDeclaration('var', [t.variableDeclarator(t.identifier(path.node.local.name), buildClassNameToScopeNameMap(tokens))]);

                    if (thisPluginOptions.keepImport) {
                        path.parentPath.replaceWithMultiple([t.importDeclaration([], t.stringLiteral(value)), varDeclaration]);
                    } else {
            ...
            CallExpression: function CallExpression(path, _ref5) {
                ...
                if (matchExtensions.test(stylesheetPath)) {
                    ...
                    if (!t.isExpressionStatement(path.parent)) {
                        ...
                        if (thisPluginOptions.keepImport) {
                            findExpressionStatementChild(path, t).insertBefore(t.expressionStatement(t.callExpression(t.identifier('require'), [t.stringLiteral(stylesheetPath)])));
                        }
                    } else {
                        if (!thisPluginOptions.keepImport) {
                            path.remove();
